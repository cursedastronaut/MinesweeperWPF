...
??? note Pourquoi mettre une grille dans chaque Border?
    Afin de pouvoir manupuler plus facilement les boutons et les labels qui vont ensemble, il est plus facile de les regrouper aussi dans l'arbre de l'application en les mettant enfant d'un même contrôle. Il me faut donc utiliser un contrôle de type Layout. Ici, j'utilise une grille pour ça car la grille va automatiquement s'assurer que les contrôles prennent toute la place qu'ils ont de disponible. Tu pourrais très bien utiliser un StackPanel ou un DockPanel aussi... 
